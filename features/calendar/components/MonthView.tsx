...
export const MonthView: React.FC<MonthViewProps> = ({ currentDate, appointments, onDayClick }) => {
  ...
  return (
    <div className="bg-white dark:bg-surface-900 rounded-2xl border border-surface-200 dark:border-surface-800 shadow-sm overflow-hidden animate-fade-in">
      <div className="grid grid-cols-7 border-b border-surface-200 dark:border-surface-800 bg-surface-50/50 dark:bg-surface-800/50">
        {weekDays.map(day => (
      ...
      <div className="grid grid-cols-7 auto-rows-fr bg-surface-200 dark:bg-surface-800 gap-[1px] border-b border-surface-200 dark:border-surface-800">
        {blanks.map((_, i) => (
          <div key={`blank-${i}`} className="bg-white/50 dark:bg-surface-900/50 h-20 md:h-40" />
        ))}
        ...
        {daysInMonth.map((date) => {
          ...
          return (
            <div 
              ...
              className={cn(
                "bg-white dark:bg-surface-900 p-1 md:p-3 h-24 md:h-40 relative group transition-all duration-200 hover:z-10 hover:shadow-lg cursor-pointer flex flex-col",
                isCurrentDay ? "bg-primary-50/30 dark:bg-primary-900/10" : "hover:bg-surface-50 dark:hover:bg-surface-800"
              )}
            >
